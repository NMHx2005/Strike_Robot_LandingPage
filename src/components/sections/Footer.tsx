...
export function Footer() {
  return (
    <footer
      className="relative p-12 cv-auto"
    ...
    >
      <div className="mx-auto flex flex-col items-center gap-6 md:flex-row md:justify-between">
        ...
        <p className="max-w-xs text-center text-sm leading-snug text-black/50 md:flex-1">
          {FOOTER.copyright}
        </p>

        <div
          className="flex items-center justify-end gap-3 md:w-[320px]"
          role="list"
        ...
        >
          {FOOTER.socials.map(({ label, href }) => (
            <Link
              ...
              aria-label={label}
              role="listitem"
              className="flex h-9 w-9 items-center justify-center rounded-full bg-black/55 text-white/90 transition-[background-color,opacity] duration-200 hover:bg-black/70 hover:text-white"
            >
